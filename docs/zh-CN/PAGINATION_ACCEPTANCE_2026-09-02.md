...
打开真实登录会话的 `/portal/jobs/custom` 后，页面显示 `10 条/页`、`共 0 条` 和“暂无数据”，没有前端异常弹窗。该结果表示当前页面所使用的当前用户范围在本次会话中没有可显示的 EMIAS 作业；它不是把空列表误判为接口失败。此前同一文档第 9.1 节记录的接口级真实验收（个人/账户/管理员分别为 6/8/8）仍然有效，本次页面复验额外确认了前端在当前空数据范围下的分页空态。
...
本节只记录本次重启后的真实页面证据，不替代第 9.1 节已经完成的 EMIAS 三权限范围接口验收；若要解释页面空态与接口级 `total=6` 的差异，下一步应在同一会话中导出页面实际请求参数并与 `/v1/aijobs/page` 的响应逐项对照。
